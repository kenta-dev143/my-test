...
<div class="row" style="margin-top:10px;">
  <div class="col-md-12">
    <div class="card">
    @if($error_pattern == "")
      <form name="search_form" action="?page=kaijyou_syuukei_hikaku_ruikei" method="post">
        ...
        <div>
          ...
          <ul class="list-group list-group-horizontal mb-4">
            @foreach($compare_event_syoutai_yotei_ymd_array as $ymd)
            <li class="h5 my-1">{{ $ymd }}@if(!$loop->last)、 @endif</li>
             @endforeach
          </ul>
        </div>
    ...
    @elseif($error_pattern == 3)
      <div style="padding-left: 5em;">
        <br><br><br><br>
        <span>イベント管理から(招待者)来場予定日時を設定してください</span>
        <br><br>
        @php($tmp_event_id = $select_event_rec['event_id'])
        @if(count($compare_event_syoutai_yotei_ymd_array) == 0)
          @php($tmp_event_id = $select_event_rec['event_compare_event_id'])
         @endif
        <button type="button" onClick="location.href='?page=event_edit&id={!! $tmp_event_id !!}';" class="btn btn-primary btn-round" style="width:200px;">設定を行う</button>
        <br><br><br><br><br>
      </div>
     @endif
    </div>
