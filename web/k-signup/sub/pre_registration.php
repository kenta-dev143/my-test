<?php
    ...
    $_conf_rsignup_big_cate1[8] = "(来場者)その他";
    ...
    switch( $_request['exec'] ){
        ...
        case 'pre_save':
            if( _count( $this_sess ) <= 0 ){
                ...
                $hontouroku_url = _SYSTEM_ROOT_URLS."/k-signup/".$event_rec['event_url_key']."/?exec=regist&iu=".$ango;
                ...
                // template set
                $mail_tpl = "k_signup_kari_touroku.tpl";
            ...
            if($_request['iu']==""){
                ...
                if($p_event_id != $event_rec['event_id']){
                    ...
                    if(_count($rsup_recs)==0){
                        ...
                        $comp_recs = _select(" select * from m_company where company_business_management_flg = 1 and company_delete_date is null");
                        $companies = array();
                        foreach ($comp_recs as $company)
                        {
                            $companies[$company['company_id']] = $company['company_name'];
                        }
                        $this_sess['companies'] = $companies;
        ...
        case 'confirm':
            ...
            if ( $this_sess['rsup_id'] == '' ){
                ...
                if ( ! isset($this_sess['syoutai_id']) || empty($this_sess['syoutai_id']))
                {
                    $company_recs = _select(' select * from m_company where company_id = ' . $_request['company_id'] . ' and company_delete_date is null');
                    $kigyou_name = $company_recs[0]['company_name'];
                    $this_sess['user_kigyou_name'] = $kigyou_name;
                    $this_sess['user_kigyou_name_kana'] = $company_recs[0]['company_name_kana'];
                }
                else
                {
        ...
        case 'save':
            if( _count( $this_sess ) <= 0 ){
                ...
                }elseif($this_sess['user_big_cate']=="6" || $this_sess['user_big_cate']=="8"){
                ...
                $data_rec = array();
                $data_rec['event_name']         = $event_rec['event_name'];
                $data_rec['kigyou_name']        = $company[0]['company_name'];
                ...
                // template set
                $mail_tpl = "k_signup_hon_touroku.tpl";
                ...
                //担当者にも通知
                // template set
                $mail_tpl = "k_signup_syoutai_tsuuchi.tpl";
