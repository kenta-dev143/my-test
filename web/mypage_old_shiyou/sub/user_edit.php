<?php
        ...
        if($this_sess['token'] != $_request['token']){
            ...
            if(_count($err_msg) == 0){
                ...
                if($_request['ex']=="1"){
                    ...
                    $success_msg = '<center>変更が完了いたしました<br><div class="button-area" style="margin-top:5px;padding-top:0px;"><button onClick="location.href=\''.$ex_login_url.'\';" type="submit" class="blue-button t-hover" style="width:300px;margin-top:5px;margin-bottom:10px;"><span>WEB展示会へ</span></button></div></center>';
                }else{
    ...
    if($this_sess['raijyousya_kbn']=="1"){
        ...
        for ($i=0; $i < _count($wArr); $i++) { 
            ...
            // 2021.06.01 add ---------- Start ----------
            $ymd_hi = $ymd." ".$hi;
            $_conf_raijyou_yotei_time2[$ymd_hi] = $ymd_hi;
            // 2021.06.01 add ---------- End   ----------
        }
        $sm->assign('_conf_raijyou_yotei_time',$_conf_raijyou_yotei_time);
        $sm->assign('_conf_raijyou_yotei_time2',$_conf_raijyou_yotei_time2); // 2021.06.01 add
    ...
    $sm->assign('_conf_tantousya',$_conf_tantousya);


    $sm->assign('_conf_vip',$_conf_vip);
    $sm->assign('_conf_big_cate',$_conf_big_cate);
    $sm->assign('_conf_mid_cate',$_conf_mid_cate);
    $sm->assign('_conf_big_cate1_web',$_conf_big_cate1_web);
