<?php
    ...
    $sm->assign('ymd', $today);
    $sm->assign('disp_ymd', date("n月j日",strtotime($today) )."（"._getYoubi($today)."）" );
    $sm->assign('event_rec',$event_rec);
    $sm->assign('kaitou_ok',$kaitou_ok);
    $sm->assign('kizon_kaitou',$kizon_kaitou);
    $sm->assign('exec',$_request['exec']);
