<?php
    ...
    if($_SESSION[_PROJECT_NAME]['select_event_id']==""){
        $contents_title = "会場全体集計比較";
        $error_pattern = 1;
    } else if (!is_null($select_event_rec) && empty($select_event_rec['event_compare_event_id'])) {
        $contents_title = "会場全体集計比較" . "（".$select_event_rec['event_name'] .":".$select_event_rec['event_kaijyou_name']."）";
        $error_pattern = 2;
    }else{
        $contents_title = "会場全体集計比較" . "（".$select_event_rec['event_name'] .":".$select_event_rec['event_kaijyou_name']."）";
        ...
        $hit_today_ymd = "";
        $nth_day = 1; // 今日がイベント何日目か
        $index = 0;
        while(true){
            $select_ymd_arr[$w_ymd] = date("Y年n月j日",strtotime($w_ymd))."（"._getYoubi($w_ymd)."）";
            if($w_ymd==date("Y/m/d")) {
                $hit_today_ymd = $w_ymd;
                $nth_day = $index + 1;
            }
        ...
        $w_ymd = $compare_event['event_raikainri_ymd_st'];
        $select_ymd_arr = array();
        $hit_today_ymd = "";
        $nth_compare_day = "";
        $index = 0;
        while(true){
            ...
            $w_ymd = date("Y/m/d",strtotime($w_ymd." +1 day"));
            if($w_ymd > $compare_event['event_raikainri_ymd_ed']){
                break;
            }
            $index++;
        }
        $blade->assign('compare_ymd_arr',$select_ymd_arr);

        if($this_sess['search_condition']['compare_ymd']==""){
            if($hit_today_ymd!=""){
                $this_sess['search_condition']['compare_ymd'] = $hit_today_ymd;
            }else{
                if ($nth_compare_day !== "" && $hit_today_ymd !== "") {
                    $this_sess['search_condition']['compare_ymd'] = $nth_compare_day;
                } else {
                    $this_sess['search_condition']['compare_ymd'] = $compare_event['event_raikainri_ymd_st'];
                }
            }
        }elseif($this_sess['search_condition']['compare_ymd']!="" &&
                $this_sess['search_condition']['compare_ymd'] >= $compare_event['event_raikainri_ymd_st'] &&
                $this_sess['search_condition']['compare_ymd'] <= $compare_event['event_raikainri_ymd_ed'] ){
            //OK
        }else{
            $this_sess['search_condition']['compare_ymd'] = $compare_event['event_raikainri_ymd_st'];
        }

        if( _dateCheck($this_sess['search_condition']['compare_ymd'],'')==false ){
            _dispError();
        }
        ...
        $event_summary = [
            'select' => [
                ...
                'ymd' => $this_sess['search_condition']['select_ymd'],
            ...
            'compare' => [
                ...
                'ymd' => $this_sess['search_condition']['compare_ymd'],
        ...
        foreach ($event_summary as $key => $event) {
            ...
            foreach ($_conf_jikantai as $time_key => $time_value) {
                ...
                $sql .= " and kinout_time_in >= '".$event['ymd']." ".$from.":00'";
                $sql .= " and kinout_time_in < '".$event['ymd']." ".$to.":00'";
        ...
        if ($_request['exec'] == "download_csv") {
            ...
            $csv_head = $select_event_rec['event_pulldown_name'] . '：' . $this_sess['search_condition']['select_ymd'];
            $csv_head .= "\r\n";
            $csv_head .= $compare_event['event_pulldown_name'] . '：' . $this_sess['search_condition']['compare_ymd'];
            $csv_head .= "\r\n";
            $csv_head .= "\r\n";
    ...
    $active_menu = "kaijyou_syuukei_hikaku";
    $contents_tpl = "kaijyou_syuukei_hikaku";
