...
$line_err = array();
    ...
    if( $_request['exec'] == "search"){
        ...
        unset( $this_sess['search_condition'] );
        $this_sess['search_condition']['order_by'] = "syozoku_id asc";
    ...
    // --------------------------------------------------- //
    $where = "";
    $where .= "syozoku_delete_date is null";

    // --------------------------------------------------- //
    // 検索条件
    // --------------------------------------------------- //
    if($this_sess['search_condition']['syozoku_name'] !=""){
        $where .= " and syozoku_name like '%"._as( $this_sess['search_condition']['syozoku_name'] )."%'";
    }

    if (isset($this_sess['search_condition']['hidden_flg']) && $this_sess['search_condition']['hidden_flg'] != '') {
        $where .= " and syozoku_hidden_flg = " ._as( $this_sess['search_condition']['hidden_flg'] );
    }
    ...
    if ($_request['exec'] == "download_csv") {
        ...
        $w_flnm = "支店・部署一覧_".date("YmdHis").".csv";
        ...
        while( $rec = _fetchArray( $result, $row ) ) {
            $csv_buff = '';
            $csv_buff .= '"'.csvSafe($rec['syozoku_id']).'"';
            $csv_buff .= ',"'.csvSafe($rec['syozoku_name']).'"';
            $csv_buff .= ',"'.csvSafe($rec['syozoku_code']).'"';
            $csv_buff .= ',"'.csvSafe($rec['syozoku_hidden_flg']).'"';
            $csv_buff .= "\r\n";
            ...
            if ($file_move !== false) {
                ...
                if ($fp !== false) {
                    ...
                    while (($csv_row = fgetcsv($fp)) !== FALSE) {
                        ...
                        $data_arr = array();
                        $data_arr['syozoku_id']         = trim($csv_row[0]); // ID
                        $data_arr['syozoku_code']       = trim($csv_row[2]); // コード
                        $data_arr['syozoku_hidden_flg'] = trim($csv_row[3]); // 表示状態
                        unset($csv_row);

                        if ($data_arr['syozoku_hidden_flg'] != 0 && $data_arr['syozoku_hidden_flg'] != 1) {
                        ...
                        if ( _count($w_err) == 0 ) {
                            ...
                            $sql .= " and syozoku_id = '"._as($data_arr['syozoku_id'])."'"."\n";
                            $szkgrp_recs = _select( $sql );
                            if ( _count($szkgrp_recs) == 0 ){
                                $w_err[] = "(${line}行目) 該当する支店・部署がありません。";
                            }
                        }

                        if ( _count($w_err) == 0 ) {
                            if ($data_arr['syozoku_code'] !== '') {
                                $sql = "";
                                $sql .= " select *" . "\n";
                                $sql .= " from m_syozoku" . "\n";
                                $sql .= " where syozoku_code = '" . _as($data_arr['syozoku_code']) . "'" . "\n";
                                $szkgrp_recs = _select($sql);
                                if (_count($szkgrp_recs) > 0)
                                {
                        ...
                        if( _count($line_err) > 0 ) {
                            ...
                            $array = array();
                            if ($data_arr['syozoku_code'] === '') {
                                $array['syozoku_code'] = "NULL";
                            } else {
                                $array['syozoku_code'] = "'"._as($data_arr['syozoku_code'])."'";
                            }
                            $array['syozoku_hidden_flg'] = "'"._as($data_arr['syozoku_hidden_flg'])."'";
                            $update_where = "syozoku_id = '"._as($data_arr['syozoku_id'])."'";

                            _update( 'm_syozoku', $array, $update_where);
    ...
    } else if ($_request['exec'] == "upload_code_csv") {
        setlocale(LC_ALL, 'ja_JP.UTF-8');
        if (is_uploaded_file($_FILES["code_csv_file"]["tmp_name"])) {
            set_time_limit(180); //3分起動
            ini_set('memory_limit',"1024M"); //メモリ拡大

            $extension  = '.'._get_extension($_FILES['code_csv_file']['name']);
            $temp_file  = _SYSTEM_ROOT_DIR . "/upfile/new_tmp/" . rand() . $extension;
            $file_move  = move_uploaded_file($_FILES['code_csv_file']['tmp_name'], $temp_file );
            ...
            if ($file_move !== false) {
                ...
                if ($fp !== false) {
                    ...
                    _update( 'm_syozoku', ['syozoku_hidden_flg' => "'1'"], "true");
                    ...
                    while (($csv_row = fgetcsv($fp)) !== FALSE) {
                        ...
                        $data_arr = array();
                        $area_name = trim($csv_row[0]);
                        $szkgrp_code = trim($csv_row[1]);
                        $szkgrp_name = trim($csv_row[2]);
                        $syozoku_code = trim($csv_row[3]);
                        $syozoku_name = trim($csv_row[4]);
                        unset($csv_row);
                        ...
                        $sql .= " and tanarea_name = '"._as($area_name)."'"."\n";
                        $area_recs = _select( $sql );

                        $sql = "";
                        $sql .= " select *"."\n";
                        $sql .= " from m_syozoku_group"."\n";
                        $sql .= " where szkgrp_delete_date is null"."\n";
                        $sql .= " and szkgrp_code = '"._as($szkgrp_code)."'"."\n";
                        $sql .= " and szkgrp_hidden_flg = '0'"."\n";
                        $szkgrp_recs = _select( $sql );

                        if ( _count($area_recs) == 0 ) {
                            $w_err[] = "(${line}行目) 該当するエリア（" . $area_name . "）がありません。";
                        }
                        ...
                        $sql .= " from m_syozoku"."\n";
                        $sql .= " where syozoku_delete_date is null"."\n";
                        $sql .= " and syozoku_code = '"._as($syozoku_code)."'"."\n";
                        ...
                        if( _count($line_err) > 0 ) {
                            ...
                            if ( _count($syozoku_recs) > 0 ) {
                                // 更新
                                $array = array();
                                $array['syozoku_szkgrp_id']    = "'"._as($szkgrp_recs[0]['szkgrp_id'])."'";
                                $array['syozoku_tanarea_id']   = "'"._as($area_recs[0]['tanarea_id'])."'";
                                $array['syozoku_name']         = "'"._as($syozoku_name)."'";
                                $array['syozoku_hidden_flg']   = "'0'";
                                $update_where = "syozoku_id = '"._as($syozoku_recs[0]['syozoku_id'])."'";
                                ...
                                $array['syozoku_tanarea_id']   = "'"._as($area_recs[0]['tanarea_id'])."'";
                                $array['syozoku_name']         = "'"._as($syozoku_name)."'";
                                $array['syozoku_code']         = "'"._as($syozoku_code)."'";
                                $array['syozoku_hidden_flg'] = "'0'";
                    ...
                    if( $error_line_count > 0 ) {
                        ...
                        if($update_success > 0 || $insert_success > 0){
                            $success_msg = "支店・部署の一括登録が完了しました。\n";
                            $success_msg .= "【登録】".number_format($insert_success)."件\n";
                            $success_msg .= "【更新】".number_format($update_success)."件";
                        }
                        _query($conn, "commit");
                    }
    ...
    // 件数取得SQL
    $sql  = "";
    $sql .= " select count(m_syozoku.syozoku_id) as all_cnt "."\n";
    $sql .= " from m_syozoku"."\n";
    $sql .= " where ".$where;
    $rec = _select($sql);
    ...
    $contents_title = "支店・部署管理";
    $active_menu = "syozoku_list";
    $contents_tpl = "syozoku_list";
