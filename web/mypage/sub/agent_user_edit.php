<?php
    ...
    } else if (! empty($this_sess['user_id'])) {
        $user_id = $this_sess['user_id'];
    }
    if (empty($user_id)) {
        die("System Error");
    }
...
//                                "user_kigyou_name_kana,企業名カナ"                 => "need,zenkana",
                ...
                $sql .= "   and user_id != '" . _as( $this_sess['user_id'] ) ."'";
            ...
            if(_count($err_msg) == 0){
                ...
                $where = "user_id='"._as($this_sess['user_id'])."'";
                ...
                $where = "un_user_id='"._as($this_sess['user_id'])."'";
                ...
                if($_request['ex']!="1"){
                    ...
                    if ( $user_rec[0]['user_mail'] != $this_sess['user_mail'] ){
                        ...
                        $where = "um_user_id='"._as($this_sess['user_id'])."'";
    ...
    if($_request['exec'] != 'save' && _count($err_msg) == 0){
        ...
        if($_request['ex']=="1"){
            $user_rec[0]['big_cate_name'] = $_conf_big_cate1_web[$user_rec[0]['user_big_cate']];
        }else{
            $user_rec[0]['big_cate_name'] = $_conf_big_cate[$user_rec[0]['user_big_cate']];
        }
        $user_rec[0]['mid_cate_name'] = $_conf_mid_cate[$user_rec[0]['user_mid_cate']];
        $user_rec[0]['raijyousya_kbn'] = $_conf_raijyousya_kbn[$user_rec[0]['user_big_cate']];
        $user_rec[0]['user_pass'] = "";

        $this_sess = $user_rec[0];
    ...
    $contents_tpl = "agent_user_edit.html";
