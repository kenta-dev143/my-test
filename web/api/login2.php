<?php
    ...
    if($_request['evekey']==""){
        ...
        $_request['login_mail'] = _getMailAddressFromID( $_request['login_id'] );
        // emailアドレスの形式チェック
        if ( _emailCheck($_request['login_mail'], '') === false ){
            ...
            if(count( $managechk_recs ) > 0){
                ...
                if($managechk_recs[0]['user_pass'] == md5($_request['login_pass']) ){
                    ...
                    if($managechk_recs[0]['user_web'] == "1"){
                        ...
                        // 2021/04/01 mod ------------------------ before ----------------------- 大幅変更
                        // $real_user_mail_addr = _getMailAddressFromID( $managechk_recs[0]['user_mail'] );
                        // $return_arr['data']['login_id'] = "".$managechk_recs[0]['user_mail'];
                        // $return_arr['data']['mail'] = "".$real_user_mail_addr;
                        // 2021/04/01 mod ------------------------ after ------------------------ 大幅変更
                        $return_arr['data']['login_id'] = "".$managechk_recs[0]['user_login_id'];
                        $return_arr['data']['mail']     = "".$managechk_recs[0]['user_mail'];
                        // 2021/04/01 mod ------------------------ end   ------------------------ 大幅変更
