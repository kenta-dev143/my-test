<?php
    ...
    //         $_SESSION[_PROJECT_NAME]['r_signup_test_mode'] = $_request['test_mode'];
    //     }
    //     if($_SESSION[_PROJECT_NAME]['r_signup_test_mode']=="1"){
    //         $signup_koukai_flg = 1;
    //     }
    // }
    ...
    if( _file_exists(_SYSTEM_ROOT_DIR."/r-signup/sub/" . $_request['page'] . '.php' ) ){
        $conn = _dbConnect();
        //_query( $conn, "begin" );
        ...
        if($event_rec['event_rsignup_start_ymdhi']!=""){
            if( date("Y/m/d H:i") >= $event_rec['event_rsignup_start_ymdhi'] ){
                // ---------- 本番公開 ------------
                $signup_koukai_flg = 1;
            }else{
                // ---------- テスト中 ------------
        ...
        if($event_rec['event_rsignup_end_ymdhi']!=""){
            if( date("Y/m/d H:i") < $event_rec['event_rsignup_end_ymdhi'] ){
                // ---------- 本番公開 ------------
                $signup_end_flg = 0;
            }else{
                // ---------- テスト中 ------------
        ...
        require( _SYSTEM_ROOT_DIR."/r-signup/sub/" . $_request['page'] . '.php' );
    ...
    // Bladeで画面を表示
    // ******************************************************************************************************
    $blade->template_dir = _SYSTEM_ROOT_DIR."/views/r-signup";

    if($signup_end_flg==1){
        $blade->display( "syuuryou" );
    }elseif($signup_koukai_flg==1){
