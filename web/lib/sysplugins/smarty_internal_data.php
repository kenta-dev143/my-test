...
class Smarty_Internal_Data
{
    ...
    public function assign($tpl_var, $value = null, $nocache = false)
    {
        ...
        if (is_array($tpl_var)) {
            foreach ($tpl_var as $_key => $_val) {
                ...
                // if ($_key !== '') {
                //     $_zenkaku_ari = false;
                //     if( $this->zenkakuChk($key)==true){
                //         $_zenkaku_ari = true;
                //     }
                //     if($_zenkaku_ari==true){
                //         $_w_key = base64_encode(mb_convert_encoding($_key,"UTF8","auto"));
                //         $_w_key = str_replace('/','_slashe_',$_w_key);
                //         $_w_key = str_replace('+','_plus_',$_w_key);
                //         $_w_key = str_replace('=','_equal_',$_w_key);
                //     }else{
                //         $_w_key = $_key;
                //     }
                //     $this->assign($_w_key, $_val, $nocache);
                // }
                // //2017/04/10 Mod ----------- End -----------
                $this->assign($_key, $_val, $nocache);
            }
            ...
            // if ($tpl_var != '') {
            //     $_zenkaku_ari = false;
            //     if( $this->zenkakuChk($tpl_var)==true){
            //         $_zenkaku_ari = true;
            //     }
            //     if($_zenkaku_ari==true){
            //         $_w_key = base64_encode(mb_convert_encoding($tpl_var,"UTF8","auto"));
            //         $_w_key = str_replace('/','_slashe_',$_w_key);
            //         $_w_key = str_replace('+','_plus_',$_w_key);
            //         $_w_key = str_replace('=','_equal_',$_w_key);
            //         $_w_key = "__ENC__" . $_w_key;
            //     }else{
            //         $_w_key = $tpl_var;
            //     }

            //     $this->_kanji_to_b64($value);

            //     if($_zenkaku_ari==true || ($_zenkaku_ari==false && substr($_w_key,0,7)!="__ENC__")){
            //         if ($this->_objType == 2) {
            //             /** @var  Smarty_Internal_Template $this */
            //             $this->_assignInScope($_w_key, $value, $nocache);
            //         } else {
            //             $this->tpl_vars[ $_w_key ] = new Smarty_Variable($value, $nocache);
            //         }
            //     }
            // }
            // //2017/04/10 Mod ----------- End -----------
    ...
    //漢字処理
    //全角があれば半角のｂａｓｅ６４をベースとしたものに変換する
    // function _kanji_to_b64(&$_val){
    //     if (is_array($_val)){
    ...
    //             if ($key !== '') {
    //                 $_zenkaku_ari = false;
    //                 //if(!preg_match('/^[a-zA-Z\d_\D_\-\{\}\.\[\]\$\!\%\&\(\)\=\|\+]*$/', $key)){
    //                 if( $this->zenkakuChk($key)==true){
    //                     $_zenkaku_ari = true;
    //                 }
    //                 if($_zenkaku_ari==true){
    //                     //$_w_key = base64_encode($key);
    //                     //2014/08/01 Mod
    //                     $_w_key = base64_encode(mb_convert_encoding($key,"UTF8","auto"));
    //                     $_w_key = str_replace('/','_slashe_',$_w_key);
    //                     $_w_key = str_replace('+','_plus_',$_w_key);
    //                     $_w_key = str_replace('=','_equal_',$_w_key);
    //                     $_w_key = "__ENC__" . $_w_key;
    ...
    // //全角であるかどうかの確認
    // function zenkakuChk($_val){
    //     if( (mb_strlen($_val) !=  strlen(mb_convert_encoding($_val,"SJIS-WIN","auto"))) || mb_detect_encoding($_val)=="" ){
    //         return true;
    //     }else{
    //         return false;
    //     }
