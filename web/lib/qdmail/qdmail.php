...
*/
	...
	function addressField( $addr = null , $name = null ){
		...
		if( !is_null($addr) && is_array($addr) && 1 < _count($addr) ){
	...
	function charset( $array = null , $op = null ){
		...
		if( is_array( $array ) ){
			if( 2===_count($array) && isset($array[0]) && isset($array[1])){
...
$this->debugEchoLf($this->to);
	...
	function sendBase(){
		...
		if( $fg_debug && (  ( 0 === _count( $this->error ) ) && ( 0 === _count( $this->error_stack ) ) ) || $this->ignore_error ) {
			//
...
//			$this->content_for_mailfunction,
	...
	function & serchBodyStructure( $content_type , & $bbs , & $false ){
		foreach($bbs as $fkey => $bs){
			if( isset( $bs['HEADER']) && ( 0 < _count($bs['HEADER']))) {
	...
	function buildHeader(){
		...
		foreach( $this->addr_many as $section => $many ){
			if( 0 == _count( $this->{strtolower( $section )} ) ){
				continue;
			}
	...
	function buildBody( $structure , $boundary = null , $rel = false , $boundary_fix = false){
		...
		foreach( $structure as $key => $value ){
			...
			if( is_array( $value ) ){
				...
				if( 0 == _count($ret_cont) && $structure['OMIT']){
					continue;
				}elseif( 1 == _count($ret_cont) && $structure['OMIT']){
					$one = null;
	...
	function mimeEncode( $subject , $charset , $org_charset = null , $first_line_front_words_length = 12 ) {
		...
		while( $pointer <= $max ){
			...
			if( (0!==_count($_ret) && $bs64len <= $length) || (0===_count($_ret) && $bs64len <= ($length-$first_line_front_words_length)) ){
				$pointer ++;
			}else{
